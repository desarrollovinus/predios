...
<?php foreach ($unidades_funcionales as $unidad): ?>
    <name>Unidades funcionales</name>
    <Folder>
        <name><?= substr($unidad[1][0]->ficha_predial, 0, 3)?></name>
    <!--Predio  -->
    <?php foreach ($unidad as $predioArray): ?>

        <!--Info del predio  -->
            <Placemark>
                <name>Predio <?php echo $predioArray[0]->ficha_predial; ?></name>
                <Style>
                    <!-- Color de la linea  -->
                    <LineStyle>
...
header("Content-Disposition: attachment; filename="."VINUS.kml");
  ?>
