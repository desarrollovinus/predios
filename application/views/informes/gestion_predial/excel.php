...
$objPHPExcel->getActiveSheet()->getColumnDimension('Y')->setWidth(250);
...
foreach ($this->InformesDAO->obtener_predios_agrupados() as $registro) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("H{$fila}", "$margen_inicial-$ficha->margen_final");
	$objPHPExcel->getActiveSheet()->setCellValue("I{$fila}", $ficha->numero_propietarios);
	$objPHPExcel->getActiveSheet()->setCellValue("J{$fila}", $ficha->nombre_propietario);
	$objPHPExcel->getActiveSheet()->setCellValue("K{$fila}", $ficha->documento_propietario);
	$objPHPExcel->getActiveSheet()->setCellValue("L{$fila}", $ficha->direccion);
	$objPHPExcel->getActiveSheet()->setCellValue("M{$fila}", $ficha->matricula);
	$objPHPExcel->getActiveSheet()->setCellValue("N{$fila}", $ficha->no_catastral);
	$objPHPExcel->getActiveSheet()->setCellValue("O{$fila}", $ficha->municipio);
	$objPHPExcel->getActiveSheet()->setCellValue("P{$fila}", $ficha->barrio);
	$objPHPExcel->getActiveSheet()->setCellValue("Q{$fila}", $ficha->uso_edificacion);
	$objPHPExcel->getActiveSheet()->setCellValue("R{$fila}", $ficha->uso_terreno);
	$objPHPExcel->getActiveSheet()->setCellValue("S{$fila}", $ficha->topografia);
	$objPHPExcel->getActiveSheet()->setCellValue("T{$fila}", $area_total);
	$objPHPExcel->getActiveSheet()->setCellValue("U{$fila}", $area_requerida);
	$objPHPExcel->getActiveSheet()->setCellValue("V{$fila}", $area_remanente);
	$objPHPExcel->getActiveSheet()->setCellValue("W{$fila}", "=T{$fila}-X{$fila}");
	$objPHPExcel->getActiveSheet()->setCellValue("X{$fila}", "=U{$fila}+V{$fila}");
	$objPHPExcel->getActiveSheet()->setCellValue("Y{$fila}", utf8_decode($ficha->lind_titulo));
